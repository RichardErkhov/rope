...
def coordinator():
    ...
    if len(action) > 0:
        match action[0][0]:
            ...
            case _:
                print(f"Action not found: {action[0][0]} {action[0][1]}")
...
def load_swapper_model():    
    ...
    model = onnx.load("weights/inswapper_128.fp16.onnx")
    graph = model.graph
    ...
    return onnxruntime.InferenceSession("weights/inswapper_128.fp16.onnx", opts, providers=["CUDAExecutionProvider"]), emap
...
def load_GFPGAN_model():
    ...
    GFPGAN_session = onnxruntime.InferenceSession( "weights/GFPGANv1.4.onnx", providers=["CUDAExecutionProvider"])
